---
name: spec-edit
description: Use before editing any file under `spec/**`, or when persisting a design decision the user just made. Covers concern-matching, cross-reference-don't-restate, the archive rule, size-watch<!-- specflow:full-only:start -->, and propagation to BUILD_QUEUE.md<!-- specflow:full-only:end -->.
---
...
→ move stale content to `archive.md`.<!-- specflow:full-only:start --> When persisting a design
